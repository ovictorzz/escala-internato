const CACHE_VERSION = 't7a-push-v5';
...
self.addEventListener('fetch', event => {
  ...
  event.respondWith((async()=>{
    ...
    const css = `<style id="mobile-account-controls-v5">
...
html:not(.dark) #mobile-menu-stable .mms-account-v4 button{color:#334155;background:rgba(15,23,42,.035)}
    ...
    const js = `<script id="mobile-account-controls-v5-script">
(()=>{
 const irParaLogin=()=>{
  try {
 ...
 new MutationObserver(mount).observe(document.documentElement,{childList:true,subtree:true});
    ...
    if(!html.includes('mobile-account-controls-v5')) html=html.replace('</head>',css+'</head>').replace('</body>',js+'</body>');

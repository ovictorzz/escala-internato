...
if marker in s:
    print("Schedule patch already present.")
    raise SystemExit(0)
...
// Preserva compromissos acadêmicos fixos e, onde foi indicado "mantém/continua", conserva o dia existente.
...
// Para os dias informados, tudo que não foi enviado fica livre, exceto compromissos acadêmicos fixos.
for (let i=1;i<=10;i++) {
...
// Garante horários-base antes da rotina de horários especiais (PS FICHA, PS BOX e Amb UNIEURO).
for (let i=1;i<=10;i++) {
  const idTrio=String(i);
  datasAssistenciais01a06Setembro.forEach(data=>{
    if (manterDiasSetembro[idTrio]?.has(data)) return;
...
// Não existe escala assistencial publicada após 06/09; deixa o restante livre,
// preservando apenas compromissos acadêmicos fixos já cadastrados.
for (let i=1;i<=10;i++) {
  const idTrio=String(i);
...
print("September 01-06 schedule patch applied.")
